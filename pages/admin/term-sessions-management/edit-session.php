...
$session = ['id' => 1, 'name' => '2024/2025', 'start_date' => '2024-09-01', 'end_date' => '2025-08-31', 'status' => 'Active'];
...
<body class="bg-gray-50">
    ...
    <div class="max-w-2xl mx-auto px-4 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">Update session details</p>
        </div>
        ...
        <div class="bg-green-50 border-l-4 border-green-500 p-4 mb-6 rounded">
            <div class="flex items-center gap-2">
                ...
                <p class="text-green-700 font-medium">Session updated successfully!</p>
            </div>
        </div>
        <?php endif; ?>
            ...
            <form method="POST" class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" 
                           ...
                           value="<?php echo $_POST['name'] ?? $session['name'] ?? ''; ?>">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date" name="start_date" 
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-900"
                               value="<?php echo $_POST['start_date'] ?? $session['start_date'] ?? ''; ?>">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-blue-900 mb-2">
                        ...
                        <input type="date" name="end_date" 
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-900"
                               value="<?php echo $_POST['end_date'] ?? $session['end_date'] ?? ''; ?>">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-900">
                        <option value="Pending" <?php echo ($_POST['status'] ?? $session['status'] ?? '') === 'Pending' ? 'selected' : ''; ?>>Pending</option>
                        <option value="Active" <?php echo ($_POST['status'] ?? $session['status'] ?? '') === 'Active' ? 'selected' : ''; ?>>Active</option>
                        <option value="Completed" <?php echo ($_POST['status'] ?? $session['status'] ?? '') === 'Completed' ? 'selected' : ''; ?>>Completed</option>
                    </select>
                </div>
