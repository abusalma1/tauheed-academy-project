...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("
        SELECT id 
        FROM terms 
        WHERE name = ? AND session_id = ? AND deleted_at IS NULL
    ");
    $stmt->execute([$name, $session_id]);
    if ($stmt->fetch()) {
        $errors['nameError'] = "A term with this name already exists in this session.";
    }
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO terms (name, start_date, end_date, session_id, created_at, updated_at)
                VALUES (?, ?, ?, ?, NOW(), NOW())
            ");
