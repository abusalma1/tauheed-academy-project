...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($id)) $errors['id'] = 'Session Not Found';

  if(empty($errors)){
    $stmt = $conn->prepare("UPDATE sessions set deleted_at = NOW() where id =?");
    $stmt->bind_param('i', $id);
    ...
    if ($stmt->execute()) {
      ...
      echo "<script>alert('Failed to delete a Session: " . $stmt->error . "');</script>";
    }
  } else {
