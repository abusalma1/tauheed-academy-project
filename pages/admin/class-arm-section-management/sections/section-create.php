...
/* ------------------------------
   COUNTS (HELPERS ALREADY FILTER DELETED ROWS)
------------------------------ */
...
<<<<<<< HEAD
    ...
    $description = htmlspecialchars(trim($_POST['sectionDescription'] ?? ''), ENT_QUOTES, 'UTF-8');
    $headTeacher = (int) ($_POST['sectionHead'] ?? 0);
...
<<<<<<< HEAD
        ...
        ");
        $checkTeacher->execute([$headTeacher]);

        if (!$checkTeacher->fetch()) {
            $headTeacherError = "Invalid or deleted teacher selected";
        }
    }
    ...
    if (empty($nameError) && empty($descriptionError) && empty($headTeacherError)) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO sections (name, description, head_teacher_id, created_at, updated_at)
            VALUES (?, ?, ?, NOW(), NOW())
        ");
