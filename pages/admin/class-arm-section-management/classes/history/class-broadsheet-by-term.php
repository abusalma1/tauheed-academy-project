...
if (!$currentSession) {
    $_SESSION['failure'] = "Session not found or deleted";
    header('Location: ' . route('back'));
    exit();
}
...
if (!$class) {
    $_SESSION['failure'] = "Class not found or deleted";
    header('Location: ' . route('back'));
    exit();
}

/* ------------------------------
   FETCH ARM (ACTIVE ONLY)
------------------------------ */
...
if (!$arm) {
    $_SESSION['failure'] = "Class arm not found or deleted";
    header('Location: ' . route('back'));
    exit();
}
...
$stmt = $pdo->prepare("
    ...
    ORDER BY t.id ASC, str.position_in_class ASC, s.name ASC
");
