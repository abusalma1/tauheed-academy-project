...
if (isset($_GET['class_id']) && isset($_GET['arm_id'])) {
    ...
    // Ensure soft-deleted rows are excluded
    $stmt = $pdo->prepare("
        SELECT
        ...
        LEFT JOIN class_arms 
            ON class_arms.id = class_class_arms.arm_id
            AND class_arms.deleted_at IS NULL
        WHERE class_class_arms.class_id = ?
          AND class_class_arms.arm_id = ?
...
$teachers = [];
$teacherStmt = $pdo->prepare("SELECT id, name FROM teachers WHERE deleted_at IS NULL ORDER BY name ASC");
$teacherStmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($teacher_id === null) {
        ...
        // Ensure selected teacher is not deleted
        $checkTeacher = $pdo->prepare("SELECT id FROM teachers WHERE id = ? AND deleted_at IS NULL");
        $checkTeacher->execute([$teacher_id]);

        if (!$checkTeacher->fetch()) {
            $errors['general'] = "Invalid teacher selected.";
        }
    }
