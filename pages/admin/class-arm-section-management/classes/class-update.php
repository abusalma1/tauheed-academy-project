...
/* ------------------------------
   FETCH ACTIVE SECTIONS, CLASSES, ARMS
------------------------------ */

$sections = selectAllData('sections'); // must internally filter deleted_at
$classes  = selectAllData('classes', null, $class_id); // must internally filter deleted_at
$class_arms = selectAllData('class_arms'); // must internally filter deleted_at

/* ------------------------------
   FETCH CURRENTLY LINKED ARMS
------------------------------ */
...
$linked_class_arms = array_column($stmt->fetchAll(PDO::FETCH_ASSOC), 'arm_id');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $updateStmt = $pdo->prepare("
            UPDATE classes 
            SET name = ?, level = ?, section_id = ?, updated_at = NOW() 
            WHERE id = ? AND deleted_at IS NULL
        ");
        ...
        if ($success) {
            ...
            // Remove old arm links (soft-delete safe)
            $delStmt = $pdo->prepare("
                DELETE FROM class_class_arms 
                WHERE class_id = ?
            ");
            $delStmt->execute([$id]);

            // Insert new arm links
            $insert_stmt = $pdo->prepare("
                INSERT INTO class_class_arms (class_id, arm_id, created_at, updated_at) 
                VALUES (?, ?, NOW(), NOW())
            ");
