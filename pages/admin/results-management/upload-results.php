    ...
    try {
        ...
        foreach ($ca_scores as $student_id => $ca_value) {
            ...
            $stmt = $pdo->prepare("
                ...
                ON DUPLICATE KEY UPDATE
                    ...
                    updated_at = NOW()
            ");
            $stmt->execute([$student_term_record_id, $subject_id, $ca, $exam, $total, $grade, $remark]);
        }
        ...
        foreach ($session_records as $record) {
            ...
            $stmt = $pdo->prepare("
                ...
                WHERE id = ?
            ");
            $stmt->execute([$session_total, $session_average, $scr_id]);
        }
