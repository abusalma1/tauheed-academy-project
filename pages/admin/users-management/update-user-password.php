...
$allowedTypes = ['admin', 'teacher', 'guardian', 'student'];
...
$table = $user_type . "s"; // admins, teachers, guardians, students

// Prevent resetting super admin password
if ($user_type === 'admin') {
    $stmt = $pdo->prepare("SELECT id, name, type FROM admins WHERE id = ? AND deleted_at IS NULL");
    $stmt->execute([$id]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($user && $user['type'] === 'superAdmin') {
        $_SESSION['failure'] = "Super Admin password cannot be reset.";
        header("Location: " . route('back'));
        exit();
    }
} else {
    $stmt = $pdo->prepare("SELECT id, name FROM `$table` WHERE id = ? AND deleted_at IS NULL");
    $stmt->execute([$id]);
    $user = $stmt->fetch(PDO::FETCH_ASSOC);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Ensure ID matches
    if ($postedId !== $id) {
        $errors['idError'] = "Invalid user reference.";
    }
                ...
                <div class="md:col-span-1">
                    <div class="bg-blue-50 rounded-lg shadow p-6 border-l-4 border-blue-900">
                        <h3 class="text-lg font-bold text-gray-900 mb-4">
                            <i class="fas fa-info-circle text-blue-900 mr-2"></i>Password Update Guidelines
                        </h3>
                        <ul class="space-y-3 text-sm text-gray-700">
                            <li class="flex gap-2">
                                <i class="fas fa-check text-green-600 mt-1"></i>
                                <span>Provide A new password</span>
                            </li>
                            <li class="flex gap-2">
                                <i class="fas fa-check text-green-600 mt-1"></i>
                                <span>Confirm it, to continue</span>
                            </li>
