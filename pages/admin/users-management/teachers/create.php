...
$lastStaffNumber = $lastStaffNumberRow
    ? $lastStaffNumberRow['staff_no']
    : 'No teacher account exists. Check the teachers list below the form.';

/* ------------------------------
   ACTIVE TEACHERS & COUNT
------------------------------ */

$teachers = selectAllData('teachers', true);
$teachersCount = countDataTotal('teachers', true)['total'];

/* ------------------------------
   INITIAL STATE
------------------------------ */

$name = $email = $phone = $address = $staffNumber = $qualification = $subject = $status = $gender = $experience = '';
$errors = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (!validateEmail($emailRaw)) {
        $errors['emailError'] = 'Invalid email format';
    } elseif (emailExist($emailRaw, 'teachers')) { // your rule: even deleted still block
    ...
    if ($subject === '') {
        $errors['subjectError'] = 'Subject/Department is required';
    }
...
<<<<<<< HEAD
    ...
    if ($experience === '') {
        $errors['experienceError'] = 'Experience is required';
    }
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO teachers 
                    (name, email, phone, address, staff_no, qualification, gender, experience, status, password, subject, created_at, updated_at)
                VALUES 
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
...
<body class="bg-gray-50">
    ...
    <section class="bg-gradient-to-r from-blue-900 to-blue-700   text-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <p class="text-xl text-blue-200">Create teacher accounts</p>
        </div>
    </section>
