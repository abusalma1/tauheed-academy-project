...
<<<<<<< HEAD
    ...
    if ($class !== '') {
        ...
        if (count($parts) === 2) {
            ...
            $errors['class'] = "Invalid General Studies class selection.";
        }
    }
    ...
    if ($email !== '') {
        ...
        } elseif (emailExist($email, 'students', $id)) { // your rule: even deleted still block
...
<<<<<<< HEAD
    ...
    if (empty($errors)) {
        ...
        // General class & arm (if provided)
        if (!empty($class_id) && !empty($arm_id)) {
            $stmt = $pdo->prepare("SELECT id FROM classes WHERE id = ? AND deleted_at IS NULL");
            $stmt->execute([$class_id]);
            if (!$stmt->fetch()) {
                $errors['class'] = "Selected General Studies class is invalid.";
            }

            $stmt = $pdo->prepare("SELECT id FROM class_arms WHERE id = ? AND deleted_at IS NULL");
            $stmt->execute([$arm_id]);
            if (!$stmt->fetch()) {
                $errors['class'] = "Selected General Studies class arm is invalid.";
            }
        }

        // Islamiyya class & arm (if provided)
        if (!empty($islamiyya_class_id) && !empty($islamiyya_arm_id)) {
            $stmt = $pdo->prepare("SELECT id FROM islamiyya_classes WHERE id = ? AND deleted_at IS NULL");
            $stmt->execute([$islamiyya_class_id]);
            if (!$stmt->fetch()) {
                $errors['islamiyyaClass'] = "Selected Islamiyya class is invalid.";
            }

            $stmt = $pdo->prepare("SELECT id FROM islamiyya_class_arms WHERE id = ? AND deleted_at IS NULL");
...
<<<<<<< HEAD
            ...
            // One active record per (student, session) for General Studies
            if (!empty($class_id) && !empty($arm_id)) {

                // Find existing class record for this student + session
                $stmt = $pdo->prepare("
                    SELECT * 
                    FROM student_class_records 
                    WHERE student_id = ? AND session_id = ? AND deleted_at IS NULL
                    ORDER BY id DESC
                    LIMIT 1
                ");
                ...
                if ($student_class_record) {
                    $student_class_record_id = (int)$student_class_record['id'];
                    $stmt = $pdo->prepare("
                        UPDATE student_class_records
                        SET class_id = ?, arm_id = ?, session_id = ?, updated_at = NOW()
                        WHERE id = ?
                    ");
                    ...
                    $stmt = $pdo->prepare("
                        INSERT INTO student_class_records 
                            (student_id, class_id, arm_id, session_id, created_at, updated_at) 
                        VALUES (?, ?, ?, ?, NOW(), NOW())
                    ");
...
<<<<<<< HEAD
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ?
                    ");
                    $stmt->execute([$term_id, $student_term_record['id']]);
                } else {
                    $stmt = $pdo->prepare("
                        INSERT INTO student_term_records 
                            (student_class_record_id, term_id, created_at, updated_at) 
                        VALUES (?, ?, NOW(), NOW())
                    ");
                    $stmt->execute([$student_class_record_id, $term_id]);
                }
                ...
                // If General Studies track removed, you can choose to keep or soft-delete historical records.
                // Example: soft-delete active class/term records for this student+session:
                /*
                $stmt = $pdo->prepare("
                    UPDATE student_class_records 
...
<<<<<<< HEAD
                ...
                if ($islamiyya_class_record) {
                    $islamiyya_class_record_id = (int)$islamiyya_class_record['id'];
                    $stmt = $pdo->prepare("
                    ...
                    $stmt = $pdo->prepare("
                        INSERT INTO islamiyya_student_class_records 
                            (student_id, class_id, arm_id, session_id, created_at, updated_at) 
                        VALUES (?, ?, ?, ?, NOW(), NOW())
                    ");
                ...
                $stmt = $pdo->prepare("
                    SELECT * 
                    FROM islamiyya_student_term_records 
                    WHERE student_class_record_id = ? AND term_id = ? AND deleted_at IS NULL
                    LIMIT 1
                ");
=======
...
<<<<<<< HEAD
                    $stmt = $pdo->prepare("
                        ...
                        WHERE id = ?
                    ");
                    $stmt->execute([$term_id, $islamiyya_term_record['id']]);
                } else {
                    $stmt = $pdo->prepare("
                        INSERT INTO islamiyya_student_term_records 
                            (student_class_record_id, term_id, created_at, updated_at) 
                        VALUES (?, ?, NOW(), NOW())
                    ");
                    $stmt->execute([$islamiyya_class_record_id, $term_id]);
                ...
                // If Islamiyya track removed, optionally soft-delete active records:
                /*
                $stmt = $pdo->prepare("
                    UPDATE islamiyya_student_class_records 
                    SET deleted_at = NOW(), updated_at = NOW()
...
<<<<<<< HEAD
            ...
            // Rollback on error
            $pdo->rollBack();
            echo "<p class='text-red-500'>Error updating record: " . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8') . "</p>";
        }
