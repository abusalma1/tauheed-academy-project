...
$name = $email = $phone = $address = $occupation = $relationship = $status = $confirmPassword = $password = $hashed_password = $gender = '';
$errors = [];
...
------------------------------ */
    ...
    } elseif (emailExist($email, 'guardians', null)) { // true = ignore soft-deleted
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO guardians 
                    (name, email, gender, phone, occupation, address, relationship, status, password, created_at, updated_at)
                VALUES 
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
