...
<<<<<<< HEAD
/* ------------------------------
   FETCH LAST ACTIVE STAFF NUMBER
------------------------------ */
...
$admins = selectAllData('admins', true);

// Count total active admins
$adminsCount = countDataTotal('admins', true)['total'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (emailExist($email, 'admins', true)) { // true = ignore soft-deleted
    ...
    } elseif (staffNumberExist($staffNumber, 'admins', true)) { // true = ignore soft-deleted
...
<<<<<<< HEAD
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO admins 
                    (name, email, phone, department, address, staff_no, status, type, gender, qualification, experience, password, created_at, updated_at) 
                VALUES 
                    (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
            ");
...
<body class="bg-gray-50">
    ...
    <section class="bg-gradient-to-r from-purple-900 to-purple-700 text-white py-12">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 class="text-4xl md:text-5xl font-bold mb-4">Create Admin & Super User Account</h1>

            <p class="text-xl text-purple-200">Create administrator accounts</p>
        </div>
    </section>
                            ...
                            <?php if (count($admins) > 0) : ?>
                                <?php foreach ($admins as $admin): ?>
                                    <tr class="hover:bg-gray-50">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-900"><?= htmlspecialchars($admin['staff_no'] ?? '') ?></td>
