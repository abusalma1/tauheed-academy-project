...
if (isset($_GET['id'], $_GET['table'], $_GET['type'])) {
  ...
  if (!in_array($table, $allowedTables, true)) {
    $_SESSION['failure'] = "Invalid table reference.";
  ...
  if ($table === 'admins') {
    $stmt = $pdo->prepare("SELECT type FROM admins WHERE id = ? AND deleted_at IS NULL");
    $stmt->execute([$id]);
    $admin = $stmt->fetch(PDO::FETCH_ASSOC);

    if ($admin && $admin['type'] === 'superAdmin') {
      $_SESSION['failure'] = "Super Admin accounts cannot be deleted.";
      header("Location: " . route('back'));
      exit();
    }
  }
